...
- **Workspace bash CAN'T `git push` (no GitHub creds in the Linux mount).** Push fails with `could not read Username for 'https://github.com': No such device or address`. Pattern: stage + commit from workspace bash (faster), then push via Windows .bat through Desktop Commander. Or just do everything via .bat if commits will need a `--no-verify` or anything OneDrive-aware.
- **OneDrive holds `.git/HEAD.lock` and `.git/index.lock` open**, can't be deleted from the Linux mount ("Operation not permitted"). After a failed git operation, clear with `del /F /Q .git\HEAD.lock` from a Windows .bat — Linux `rm` won't work even with `-f`.
